...
for j in range(len(accessions)):
    ...
    for k in range(len(accessions)):
        ...
        m = accessions[k]
        #print("\nM: ", m)
        z1 = ttest_ind(df_Entropies[l], df_Entropies[m])
...
#ax = df_Mismatch['Mismatch'].plot.bar(x = len(consensus), cmap = 'BrBG')

#ax = sns.scatterplot(x = position, y = values, cmap = 'BrBG' )
...
#ax = sns.scatterplot(data = df_Mismatch, x = 'Consensus', y = 'Mismatch')

# stem function: first way
#ax = plt.stem(position, values)

#ax = sns.scatter(x=df_Mismatch.index, y=df_Mismatch.Mismatch, )
...
az = sns.heatmap(df_Student_T,  annot = True, cmap = 'GnBu')
#cmap = 'GnBu',
plt.title("Heat map of p-value for Student-T cross-comparison of entropies")
...
if number_of_LCRs >= 1:
    plt.figure(figsize=(10,10))
    az1 = sns.heatmap(df_Student_T_LCR1, annot = True, cmap = 'GnBu')
...
if number_of_LCRs >= 2:
    plt.figure(figsize=(10,10))
    az2 = sns.heatmap(df_Student_T_LCR2, annot = True, cmap = 'GnBu')
...
if number_of_LCRs >= 3:
    plt.figure(figsize=(10,10))
    az3 = sns.heatmap(df_Student_T_LCR3, annot = True, cmap = 'GnBu')
...
if number_of_LCRs >= 4:
    plt.figure(figsize=(10,10))
    az4 = sns.heatmap(df_Student_T_LCR4, annot = True, cmap = 'GnBu')
...
if number_of_LCRs >= 5:
    plt.figure(figsize=(10,10))
    az5 = sns.heatmap(df_Student_T_LCR5, annot = True, cmap = 'GnBu')
...
if number_of_LCRs >= 6:
    plt.figure(figsize=(10,10))
    az6= sns.heatmap(df_Student_T_LCR6, annot = True, cmap = 'GnBu')
...
if number_of_LCRs >= 7:
    plt.figure(figsize=(10,10))
    az7= sns.heatmap(df_Student_T_LCR7, annot = True, cmap = 'GnBu')
...
if number_of_LCRs >= 8:
    plt.figure(figsize=(10,10))
    az8= sns.heatmap(df_Student_T_LCR8, annot = True, cmap = 'GnBu')
